  ...
  const [{ data: membership }, { data: match, error: matchError }] =
    await Promise.all([
      ...
      supabase
        ...
        .select(
          "id,tournament_id,event_id,home_team_id,away_team_id,stage,round_number,position,tipoff_at,court,status,home_score,away_score,winner_team_id,next_match_id,next_match_slot,started_at,ended_at,control_version,updated_at,tournament:basketball_tournaments!inner(name)",
        )
        .eq("id", matchId)
        .eq("event_id", eventId)
        .single(),
