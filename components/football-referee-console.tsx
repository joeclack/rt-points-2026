...
function ScoreSide({
  ...
  const canScore = isLiveFootballMatch(match.status) && canControl;
  ...
  return (
    <div className="flex min-w-0 flex-col items-center px-2 py-5">
      ...
      {canScore ? (
        <div className="mt-3 grid w-full max-w-36 grid-cols-2 gap-2">
          {[-1, 1].map((delta) => (
            <form action={adjustScore} key={delta}>
              ...
              <button
                ...
                className={cn(
                  "flex h-14 w-full items-center justify-center border text-white transition-colors disabled:opacity-35",
                  delta > 0
                    ? "border-emerald-500 bg-emerald-500/15 active:bg-emerald-500/30"
                    : "border-zinc-700 bg-zinc-900 active:bg-zinc-800",
                )}
                disabled={
                  !isOnline || pending || (delta < 0 && score === 0)
                }
                title={delta > 0 ? "Add goal" : "Remove goal"}
                type="submit"
              >
                {delta > 0 ? (
...
export function FootballRefereeConsole({
  error,
  eventId,
  match,
  matchMinutes,
  ...
  const clock = getFootballClock(match, matchMinutes, now);
  ...
  const activeStoppageSeconds = match.stoppageStartedAt
    ? Math.max(
        0,
        Math.floor((now - new Date(match.stoppageStartedAt).getTime()) / 1000),
      )
    : 0;
  const halfMinutes = matchMinutes / 2;
  const periodLabel = match.secondHalfStartedAt ? "SECOND HALF" : "FIRST HALF";
  const mainClock =
    ...
    <div className="fixed inset-0 z-[100] overflow-y-auto bg-[#050706] text-white [padding-bottom:env(safe-area-inset-bottom)] [padding-top:env(safe-area-inset-top)]">
      <div className="mx-auto flex min-h-[100dvh] w-full max-w-xl flex-col">
        ...
        <section className="border-b border-zinc-800 px-4 pb-5 pt-4 text-center">
          ...
          {clock?.isInAddedTime ? (
            <p className="mt-2 text-xs font-bold uppercase text-amber-300">
              Minimum {clock.addedTimeNeededLabel}
            </p>
          ) : (
        ...
        <section className="grid grid-cols-2 border-b border-zinc-800">
          <div className="border-r border-zinc-800 px-3 py-3 text-center">
            <p className="text-[0.65rem] font-bold uppercase text-zinc-500">
              Added needed
            </p>
            <p className="mt-1 font-mono text-2xl font-bold tabular-nums text-amber-300">
              {clock?.addedTimeNeededLabel ??
                `+${formatFootballDuration(
          ...
          <div className="px-3 py-3 text-center">
            ...
            <p
              className={cn(
                ...
                match.stoppageStartedAt ? "text-red-400" : "text-zinc-600",
              )}
            >
              +{formatFootballDuration(activeStoppageSeconds)}
            </p>
          </div>
        </section>
        ...
        <div className="mt-auto grid gap-2 p-3">
          {!hasControl ? (
            <form
              ...
              onSubmit={(event) => {
                if (
                  controlledByAnother &&
                  !window.confirm("Take match control from the other referee device?")
                ) {
            ...
            >
              ...
              >
                ...
                {controlledByAnother ? "Take match control" : "Claim match control"}
              </RefSubmitButton>
            </form>
          ) : null}

          {hasControl && match.status === "scheduled" ? (
            <form action={updateFootballMatchLifecycle} onSubmit={prepareCommand}>
              <MatchFields deviceId={deviceId} eventId={eventId} match={match} />
          ...
          {hasControl && match.status === "live" ? (
            <form action={updateFootballMatchLifecycle} onSubmit={prepareCommand}>
              ...
              <input
                ...
                type="hidden"
                value={
                  match.stoppageStartedAt ? "end_stoppage" : "start_stoppage"
                }
              />
              <RefSubmitButton
                className={cn(
              ...
              >
                ...
                {match.stoppageStartedAt ? "Stop stoppage" : "Track stoppage"}
              </RefSubmitButton>
            </form>
          ) : null}

          {hasControl && match.status === "halftime" ? (
            <form action={updateFootballMatchLifecycle} onSubmit={prepareCommand}>
              <MatchFields deviceId={deviceId} eventId={eventId} match={match} />
              <input name="command" type="hidden" value="start_second_half" />
              <RefSubmitButton
                className="border-emerald-400 bg-emerald-500/20 text-emerald-200"
                disabled={!isOnline}
                pendingLabel="Starting..."
          ...
          {hasControl && match.status === "live" ? (
            <form
              action={updateFootballMatchLifecycle}
              onSubmit={(event) => {
                const label = match.secondHalfStartedAt ? "full-time" : "half-time";
                if (!window.confirm(`Set ${label}?`)) {
                  event.preventDefault();
                  return;
                }
                prepareCommand(event);
            ...
            >
              ...
              <input
                name="command"
                type="hidden"
                value={match.secondHalfStartedAt ? "finish" : "halftime"}
              />
              <RefSubmitButton
              ...
              >
                ...
                {match.secondHalfStartedAt ? "End match" : "End first half"}
              </RefSubmitButton>
            </form>
          ) : null}
