...
const basketballFormatLabels: Record<BasketballFormat, string> = {
  knockout: "Knockout",
  league: "League",
};

type BasketballLiveCentreProps = {
  accessCode: string;
  eventId: string;
  eventName: string;
  eventSlug: string;
...
function mapPublicBasketball(payload: PublicBasketballPayload) {
...
function formatTipoff(iso: string | null) {
  if (!iso) {
    return "TBC";
  }
...
function gameClock(startedAt: string | null, gameMinutes: number, now: number | null) {
  if (!now) {
    return "0:00";
  }
...
function isAllocatedMatch(match: BasketballMatch, teams: Team[]) {
  return Boolean(
    match.homeTeamId &&
      match.awayTeamId &&
        ...
        <div className="mx-auto flex min-h-dvh w-full max-w-5xl flex-col px-4 py-5 sm:px-6 lg:px-8">
            ...
            <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-3">
              ...
              <div className="text-center">
                ...
                <p className="mt-1 text-xs uppercase tracking-wide text-slate-300">
                  {isResult ? "Final" : formatTipoff(match.tipoffAt)}
                </p>
              </div>
              <div className="min-w-0 text-center">
                <TeamBadge
          ...
          <div className="mb-5 border-y border-slate-200 text-sm">
            <div className="flex items-center justify-between gap-4 py-2.5">
              ...
              <p className="font-semibold text-slate-950">{gameMinutes} minutes</p>
            </div>
          </div>
      ...
      <div className="grid grid-cols-[1fr_auto_1fr] items-center border-b border-slate-200 bg-slate-50 px-4 py-2.5">
        ...
        <div className="text-center">
          <p className="font-mono text-base font-semibold tabular-nums text-slate-950">
            {gameClock(match.startedAt, gameMinutes, clockNow)}
          </p>
...
function MatchRow({
  ...
  return (
    <MatchDetailDialog gameMinutes={gameMinutes} match={match} teams={teams}>
      <button
      ...
      >
        <div className="text-xs text-slate-500">
          <p className="font-medium text-slate-700">
            {isResult ? "FT" : formatTipoff(match.tipoffAt)}
          </p>
...
export function BasketballLiveCentre({
  ...
  const upcomingMatches = useMemo(
    () =>
      tournament?.matches
        ...
        .sort(
          (a, b) =>
            (a.tipoffAt ? new Date(a.tipoffAt).getTime() : Infinity) -
              (b.tipoffAt ? new Date(b.tipoffAt).getTime() : Infinity) ||
  ...
  useEffect(() => {
    ...
    async function refreshBasketball(updatedMatchId?: string) {
      try {
        ...
        setTournaments(
          mapPublicBasketball(data as unknown as PublicBasketballPayload),
        );
    ...
    function queueRefresh(matchId?: string) {
      ...
      refreshTimeout = window.setTimeout(() => {
        void refreshBasketball(matchId);
      }, 100);
    }
        ...
        {!tournament ? (
          <section className="rounded-lg border border-slate-200 bg-white px-5 py-14 text-center">
            ...
            <p className="mx-auto mt-2 max-w-sm text-sm text-slate-500">
              The organisers have not published a basketball tournament yet.
            ...
            <section className="mb-6 flex items-end justify-between gap-4">
              <div className="min-w-0">
                <div className="flex items-center gap-1.5 text-xs font-medium text-slate-500">
                  ...
                  {basketballFormatLabels[tournament.format]}
                </div>
                ...
                <div className="rounded-lg border border-slate-200 bg-white px-4 py-4">
                  ...
                  {upcomingMatches[0] ? (
                    <p className="mt-1.5 pl-6 text-xs text-slate-500">
                      Next fixture: {formatTipoff(upcomingMatches[0].tipoffAt)}
                    </p>
                  ) : null}
                </div>
              )}
