...
import { addBasketballMatch } from "@/app/admin/events/[eventId]/basketball/actions";
...
import { DeleteBasketballTournamentButton } from "@/components/delete-basketball-tournament-button";
...
import type { BasketballFormat } from "@/lib/basketball-types";
...
export default async function AdminBasketballPage({
  params,
  searchParams,
}: {
  ...
  if (event.sport !== "basketball") {
    redirect(`/admin/events/${event.id}/football`);
  }
  ...
  const matches = selectedTournament
    ? [...selectedTournament.matches].sort(
        (a, b) =>
          Number(b.status === "live") -
            Number(a.status === "live") ||
  ...
  return (
    <div className="mx-auto w-full max-w-6xl">
      ...
      <header className="mb-6 flex flex-col justify-between gap-4 border-b border-slate-200 pb-6 sm:flex-row sm:items-end">
        <div>
          <StatusPill tone="live">Basketball admin</StatusPill>
          <h1 className="mt-4 text-3xl font-bold text-slate-950">
            {event.name}
          </h1>
          <p className="mt-1 text-sm text-slate-500">
            Tournaments, fixtures and live game control.
          </p>
        </div>
        {event.status !== "finished" ? (
          <Button asChild variant="outline">
            <Link href={`/events/${event.slug}/basketball`}>
        ...
        <Card>
          ...
          <CardContent>
            <p className="mb-4 text-sm text-slate-600">
              Add at least two basketball teams from the Teams page before
              creating fixtures.
            </p>
            <Button asChild variant="outline">
          ...
          {tournaments.length > 0 ? (
            <div className="mb-6 flex gap-2 overflow-x-auto pb-1">
              {tournaments.map((tournament) => (
                ...
                >
                  <Link
                    href={`/admin/events/${event.id}/basketball?tournament=${tournament.id}`}
                  >
                    {tournament.name}
                  </Link>
                </Button>
                  ...
                  <div>
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <span className="text-xs font-bold uppercase tracking-wider text-brand-orange">
                        {basketballFormatLabels[selectedTournament.format]}
                      </span>
                    </div>
                    ...
                    <p className="mt-1 text-sm text-slate-300">
                      {tournamentTeams.length} teams · {matches.length} games
                    </p>
                  </div>
                  <div className="flex items-center gap-3">
                    <DeleteBasketballTournamentButton
                      eventId={event.id}
              ...
              <div className="mb-4 flex items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-sm text-slate-500">
                    Live games are kept at the top.
                  </p>
                </div>
              ...
              <div className="grid gap-4 lg:grid-cols-2">
                {matches.map((match) => (
                  <AdminBasketballMatchCard
                    eventId={event.id}
                    gameMinutes={selectedTournament.gameMinutes}
                    key={match.id}
                    match={match}
                    teams={event.teams}
                  />
                ...
                >
                  ...
                  <div>
                    ...
                    >
                      ...
                      <option value="friendly">Extra game</option>
                    </select>
                  </div>
                  <div>
                    <label
                      className="mb-1 block text-sm font-medium"
                      htmlFor="extra-tipoff"
                    >
                      Tip-off
                    </label>
                    <KickoffInput id="extra-tipoff" />
                  </div>
