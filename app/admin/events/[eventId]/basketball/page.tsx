...
export default async function AdminBasketballPage({ params, searchParams }: { params: Promise<{ eventId: string }>; searchParams: Promise<{ error?: string; message?: string; tournament?: string }> }) {
  ...
  return (
    <div className="mx-auto w-full max-w-5xl">
      <header className="mb-6 flex items-end justify-between gap-4 border-b border-slate-200 pb-5"><div><h1 className="text-2xl font-semibold">Basketball</h1><p className="mt-1 text-sm text-slate-500">Timed games, live point scoring and bracket progression.</p></div>{event.status !== "finished" ? <Button asChild variant="outline"><Link href={`/events/${event.slug}/basketball`}><ExternalLink className="h-4 w-4" />Public centre</Link></Button> : null}</header>
      ...
      {event.teams.length < 2 ? <Card><CardHeader><CardTitle className="text-lg">Add teams first</CardTitle></CardHeader><CardContent><p className="mb-4 text-sm text-slate-600">Basketball needs at least two approved teams.</p><Button asChild variant="outline"><Link href={`/admin/events/${event.id}/teams`} prefetch={false}>Manage teams</Link></Button></CardContent></Card> : <>
